                ...
                try
                {
                    if (nEventId != 0)
                    {
                        ...
                        string sqlselect = @"SELECT CampaignId FROM CAMPAIGN where ParentTabId = @ParentTabId and ParentFileId = 1 
                                            and status = " + CampaignStatus.MAIL_RECURRENT;
                        ...
                        result = eWCFTools.WCFEudoProcessCaller<IEudoTreatmentCampaignWCF, eOnBreakCampaignsResponse>(wcfUrl, obj => obj.PauseEtapeMarketingRecurrent
