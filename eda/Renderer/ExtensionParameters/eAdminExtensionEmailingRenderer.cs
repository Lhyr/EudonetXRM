...
namespace Com.Eudonet.Xrm.eda
{
    ...
    public class eAdminExtensionEmailingRenderer : eAdminExtensionFileRenderer
    {
        ...
        /// </summary>
        public eAdminExtensionEmailingRenderer(ePref pref, eAdminExtension extension, string initialTab)
        ...
        /// <param name="extensionFileIdFromStore">The extension file identifier from store.</param>
        /// <param name="bNoInternet">if set to <c>true</c> [b no internet].</param>
        /// <param name="initialTab">The initial tab.</param>
        /// <returns></returns>
        /// <exception cref="EudoAdminInvalidRightException"></exception>
        public static eAdminExtensionEmailingRenderer CreateAdminExtensionEmailingRenderer(ePref pref, int extensionFileIdFromStore, bool bNoInternet, string initialTab)
        {
            if (pref.User.UserLevel < UserLevel.LEV_USR_ADMIN.GetHashCode())
                throw new EudoAdminInvalidRightException();

            eAdminExtension ext = bNoInternet ? eAdminExtension.initExtensionFromJson(eUserOptionsModules.USROPT_MODULE.ADMIN_EXTENSIONS_EMAILING, pref) :
              eAdminExtension.InitFromModule(eUserOptionsModules.USROPT_MODULE.ADMIN_EXTENSIONS_EMAILING, pref, extensionFileIdFromStore);

            return new eAdminExtensionEmailingRenderer(pref, ext, initialTab);
        }
            ...
            if (base.Init())
            {
                ...
                _dicConfigAdv = eLibTools.GetConfigAdvValues(Pref,
                    new HashSet<eLibConst.CONFIGADV> {
                        eLibConst.CONFIGADV.DKIM_SIGN_ENABLED, eLibConst.CONFIGADV.DKIM_DOMAIN, eLibConst.CONFIGADV.DKIM_PRIVATE_RSA_KEY, eLibConst.CONFIGADV.DKIM_SELECTOR, eLibConst.CONFIGADV.SPOOFING_EXTENDED_DOMAIN
                    });
        ...
        /// </summary>
        /// <returns>
        /// true si le contenu a été généré avec succès
        /// </returns>
        ...
        Boolean AddSectionEmailing()
        {
            ...
            AddCheckboxOptionField(targetPanel, "chkSplittingMailBodyNcharPerLine", eResApp.GetRes(Pref, 2986), "",
                ...
                typeof(eLibConst.CONFIG_DEFAULT), _dicConfigAdv[eLibConst.CONFIGADV.SPLITTING_MAIL_BODY_NCHAR_PER_LINE] == "1");

            
            // Paramètres du serveur
            #region Param serveur - Radio button 1ère option

            Dictionary<string, string> itemsSmtpDefaultParam1 = new Dictionary<string, string>();
            itemsSmtpDefaultParam1.Add("1", eResApp.GetRes(Pref, 5046));
            AddRadioButtonOptionField(targetPanel, "rbSmtpDefaultParam1", "rbSmtpDefaultParam", eResApp.GetRes(Pref, 7911), "", eAdminUpdateProperty.CATEGORY.CONFIGDEFAULT, eLibConst.CONFIG_DEFAULT.SMTPUSEDEFAULTPARAM.GetHashCode(),
                typeof(eLibConst.CONFIG_DEFAULT), itemsSmtpDefaultParam1, _dicConfigDefault[eLibConst.CONFIG_DEFAULT.SMTPUSEDEFAULTPARAM] == "1" ? "1" : "0", EudoQuery.FieldFormat.TYP_NUMERIC,
            ...
            #region Param serveur - Radio button 2ème option

            Dictionary<string, string> itemsSmtpDefaultParam0 = new Dictionary<string, string>();
            itemsSmtpDefaultParam0.Add("0", eResApp.GetRes(Pref, 5047));
            AddRadioButtonOptionField(targetPanel, "rbSmtpDefaultParam0", "rbSmtpDefaultParam", "", "", eAdminUpdateProperty.CATEGORY.CONFIGDEFAULT, eLibConst.CONFIG_DEFAULT.SMTPUSEDEFAULTPARAM.GetHashCode(),
                typeof(eLibConst.CONFIG_DEFAULT), itemsSmtpDefaultParam0, _dicConfigDefault[eLibConst.CONFIG_DEFAULT.SMTPUSEDEFAULTPARAM] == "1" ? "1" : "0", EudoQuery.FieldFormat.TYP_NUMERIC,
                onClick: "nsAdmin.sendJson(this, false, true);nsAdmin.toggleSMTPServerParams(this);");
            ...
            AddTextboxOptionField(pnlAuth, "txtEmailingPassword", eResApp.GetRes(Pref, 2), "",
                ...
                typeof(eLibConst.CONFIG_DEFAULT), _dicConfigDefault[eLibConst.CONFIG_DEFAULT.SMTPEMAILINGPASSWORD], EudoQuery.AdminFieldType.ADM_TYPE_CHAR, eAdminTextboxField.LabelType.INLINE,
                passwordField: true,
                customPanelCSSClasses: customPanelCSSClasses, customLabelCSSClasses: customLabelCSSClasses, customTextboxCSSClasses: customTextboxCSSClasses);
