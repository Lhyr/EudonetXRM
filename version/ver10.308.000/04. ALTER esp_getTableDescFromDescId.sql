...
select @sLib as libelle, [desc].[ToolTipText], [desc].[ObligatReadOnly], [desc].[ProspectEnabled],
	...
	case 
		when P_LEFT_MENU.mode = 1 then case when charindex(';'+ cast(@userid as varchar(50)) + ';',';'+ P_LEFT_MENU.[user] +';') > 0 or charindex(';G'+cast( @groupid as varchar(50)) + ';',';'+ P_LEFT_MENU.[user] +';') > 0 then 0 else 1 end
		else 1
	end ACTION_MENU_P,
      ...
      case 
                  when P_IMPORT_TAB.mode = 0 then case when @ulevel >= P_IMPORT_TAB.level then 1 else 0 end
                  when P_IMPORT_TAB.mode = 1 then case when charindex(';'+ cast(@userid as varchar(50)) + ';',';'+ P_IMPORT_TAB.[user] +';') > 0 or charindex(';G'+cast( @groupid as varchar(50)) + ';',';'+ P_IMPORT_TAB.[user] +';') > 0 then 1 else 0 end
                  when P_IMPORT_TAB.mode = 3 then case when (@ulevel >= P_IMPORT_TAB.level) And ( charindex(';'+ cast(@userid as varchar(50)) + ';',';'+ P_IMPORT_TAB.[user] +';') > 0 or charindex(';G'+ cast( @groupid as varchar(50)) + ';',';'+ P_IMPORT_TAB.[user] +';') > 0) then 1 else 0 end
                  when P_IMPORT_TAB.mode = 2 then case when (@ulevel >= P_IMPORT_TAB.level) or ( charindex(';'+ cast(@userid as varchar(50)) + ';',';'+ P_IMPORT_TAB.[user] +';') > 0 or charindex(';G'+ cast( @groupid as varchar(50)) + ';',';'+ P_IMPORT_TAB.[user] +';') > 0) then 1 else 0 end
                  else 1 
      end IMPORT_TAB_P,

	  -- Traitements d'import en masse depuis un signet
      case 
                  when P_IMPORT_BKM.mode = 0 then case when @ulevel >= P_IMPORT_BKM.level then 1 else 0 end
                  when P_IMPORT_BKM.mode = 1 then case when charindex(';'+ cast(@userid as varchar(50)) + ';',';'+ P_IMPORT_BKM.[user] +';') > 0 or charindex(';G'+cast( @groupid as varchar(50)) + ';',';'+ P_IMPORT_BKM.[user] +';') > 0 then 1 else 0 end
                  when P_IMPORT_BKM.mode = 3 then case when (@ulevel >= P_IMPORT_BKM.level) And ( charindex(';'+ cast(@userid as varchar(50)) + ';',';'+ P_IMPORT_BKM.[user] +';') > 0 or charindex(';G'+ cast( @groupid as varchar(50)) + ';',';'+ P_IMPORT_BKM.[user] +';') > 0) then 1 else 0 end
                  when P_IMPORT_BKM.mode = 2 then case when (@ulevel >= P_IMPORT_BKM.level) or ( charindex(';'+ cast(@userid as varchar(50)) + ';',';'+ P_IMPORT_BKM.[user] +';') > 0 or charindex(';G'+ cast( @groupid as varchar(50)) + ';',';'+ P_IMPORT_BKM.[user] +';') > 0) then 1 else 0 end
                  else 1 
      end IMPORT_BKM_P,
...
from [desc] inner join [res] 
            ...
            LEFT JOIN [PERMISSION] AS P_IMPORT_TAB ON T_IMPORT_TAB.[permid] = P_IMPORT_TAB.[permissionid]        
			
			-- Import en masse depuis un signet
			LEFT JOIN [TRAIT] AS T_IMPORT_BKM ON [desc].[descid] + 19 = T_IMPORT_BKM.[traitid]
            LEFT JOIN [PERMISSION] AS P_IMPORT_BKM ON T_IMPORT_BKM.[permid] = P_IMPORT_BKM.[permissionid]               
