...
IF NOT EXISTS (
		...
		WHERE sys.tables.name LIKE 'CAMPAIGN'
			AND syscolumns.name LIKE 'PreHeader'
		)
BEGIN
	IF @bIsUnicode = 1
	BEGIN
		ALTER TABLE [CAMPAIGN] ADD [PreHeader] NVARCHAR(140) NULL;
	END
	ELSE
	BEGIN
		ALTER TABLE [CAMPAIGN] ADD [PreHeader] VARCHAR(140) NULL;
	END
END
...
IF NOT EXISTS (
		SELECT *
		FROM [DESC]
		WHERE [DescId] = @idCampaignPreHeader
		)
BEGIN
	INSERT INTO [DESC] (
		[DescId]
	...
	SELECT @idCampaignPreHeader
		...
		,[IconColor]
	FROM [DESC]
	WHERE [DescId] = @idSubjectInClear
END

IF NOT EXISTS (
		SELECT *
		FROM [RES]
		WHERE [ResId] = @idCampaignPreHeader
		)
BEGIN
	INSERT INTO [RES] (
		[ResId]
