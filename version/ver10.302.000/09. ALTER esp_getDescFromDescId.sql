...
/*
DECLARE @superadminonly AS NVARCHAR(MAX), @fieldstyle AS NVARCHAR(MAX), @valuecolor AS NVARCHAR(MAX), @labelhidden AS NVARCHAR(MAX), @maxValue AS NVARCHAR(MAX), @buttonColor AS NVARCHAR(MAX)
SELECT TOP 1 @superadminonly = Value FROM [DESCADV] WHERE Descid = @descid and Parameter = '1' 
SELECT TOP 1 @fieldstyle = Value FROM [DESCADV] WHERE Descid = @descid and Parameter = '2' 
SELECT TOP 1 @valuecolor = Value FROM [DESCADV] WHERE Descid = @descid and Parameter = '3' 
SELECT TOP 1 @labelhidden = Value FROM [DESCADV] WHERE Descid = @descid and Parameter = '4' 
SELECT TOP 1 @maxValue = Value FROM [DESCADV] WHERE Descid = @descid and Parameter = '7' 
SELECT TOP 1 @buttonColor = Value FROM [DESCADV] WHERE Descid = @descid and Parameter = '5' 
*/
...
,[desc].[case],[desc].[TreeViewUserList],[desc].[FullUserList],[desc].[AutoCompletion],
	...
	,isnull([DESC].[InterEventHidden],0), disporder, [default],[defaultformat], [Bold], [Italic], [Underline], [ForeColor], [Flat], ISNULL([TabIndex], 0) AS [TabIndex],
