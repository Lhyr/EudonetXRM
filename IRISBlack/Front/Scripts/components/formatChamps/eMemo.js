...
export default {
    ...
    mounted() {
        ...
        if (!this.propListe) {
            ...
            if (this.dataInput.IsHtml) {
                ...
                // On met Ckeditor en lecture seule si le composant l'est. Précédemment, on passait par du css mais ce n'est pas optimal
                // car cela bloque aussi le scroll si le texte dépasse
                ...
                var oInstanceReadyFct = (
                    function (vueJsObject) {
                        return function () {
                            // A l'instanciation de CKEditor, on met à jour dataInput.Value avec la valeur du champ telle que réinterprétée par CKEditor,
                            // pour ne pas considérer que la valeur ait pu changer à la sortie du champ (oBlurFct) si la seule différence entre la valeur
                            // présente en base, et la valeur renvoyée par CKEditor, est dûe à son interprétation de la valeur en HTML (ex : "" en base
                            // devient "<br>" une fois passée à CKEditor)
    ...
    computed: {
         ...
         * Utilisation d'une fonction wrapper déclarée dans XRMWrapperModules, qui utilisera le code déjà employé ailleurs dans l'application pour
    ...
    methods: {
        ...
        async openMemo(evt) {
                ...
                // Request 90 164 - Appel avec une promesse de la méthode verifComponent afin de déclencher cette dernière et udpateMethod avant de réupérer les valeurs du back via getFullMemoValue
                // #94 050 - Ne surtout pas faire ça si l'élément sur lequel on clique n'est pas un champ de saisie éditable (ex : champ Mémo affiché en signet) !
                // Sinon, la méthode verifComponent() risque d'appeler updateMethod() avec un contenu potentiellement altéré (car rendu en lecture seule), ce qui modifiera irrémédiablement le contenu du champ Mémo => PERTE DE DONNEES !
        ...
        /**
         *  # 82 638 - Récupère la valeur complète du champ Mémo en base, comme le faisait XRM/E17 (cf. eMemoDialog.aspx et eGetFieldManager.ashx)
         */
            ...
            var oScrollFct = (function (vueJsObject) {
                return function (e) {
                    ...
                    if (oScrolledObj && (
                        ...
                        // On préférera donc, ici, déclencher le onblur() de l'éditeur interne du champ Mémo pour retirer le curseur du champ et, ainsi, déclencher le masquage de la barre d'outils
                        // avec le fonctionnement interne de CKEditor.
                        // Inconvénient (ou avantage) : cela pourra déclencher la sauvegarde du champ et d'autres mécanismes lourds liés à la sortie de champ si définis.
		...
		* */
         ...
         * affichée sur le mémo dans la modale.
    ...
    // sur les champs de type Mémo, qu'on soit en lecture seule ou non, on ajoute le champ de type textarea ainsi que le bouton, pour pouvoir afficher le contenu en plein écran. L'affichage sera simplement grisé
    template: `
